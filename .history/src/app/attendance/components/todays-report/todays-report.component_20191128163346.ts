...
export class TodaysReportComponent implements OnInit {
  ...
  ngOnInit() {
    const yearLastDayEndTimes = new Date(new Date().getFullYear(), 0, 1 );
    const yearFirstDayStartTimes = new Date(new Date().getFullYear(), 11, 31);
    const startTimes = yearLastDayEndTimes.setHours(0, 0, 0, 0);
    const endTimes = yearFirstDayStartTimes.setHours(23, 59, 59, 999);
